...
from ...api_clients import GoogleAdsAPIClient, GA4APIClient
...
def export(
    ...
    date_range: str = typer.Option("30d", "--date-range", "-d", help="Date range (7d, 30d, 90d, or YYYY-MM-DD,YYYY-MM-DD)"),
    campaign_id: Optional[str] = typer.Option(None, "--campaign-id", "-c", help="Specific campaign ID to export"),
    output_file: Optional[Path] = typer.Option(None, "--output", "-o", help="Output file path"),
    format: str = typer.Option("csv", "--format", "-f", help="Output format (csv, json, xlsx)"),
    metrics: Optional[List[str]] = typer.Option(None, "--metrics", "-m", help="Specific metrics to export"),
    include_segments: bool = typer.Option(False, "--include-segments", help="Include audience segments"),
    ...
    if not output_file:
        ...
        output_file = Path(config.output.base_directory) / f"export_{source}_{timestamp}.{format}"
    ...
    with Progress(
        ...
        if source == "ga4" or source == "all":
            ...
            _export_ga4_data(config, start_date, end_date, output_file, format, metrics, campaign_id, include_segments)
            progress.remove_task(task)
        
        if source == "google-ads" or source == "all":
            task = progress.add_task("Exporting Google Ads data...", total=None)
            _export_google_ads_data(config, start_date, end_date, output_file, format, metrics, campaign_id)
            progress.remove_task(task)
...
def list_campaigns(
    ...
    status: Optional[str] = typer.Option(None, "--status", help="Filter by campaign status"),
    ...
    with Progress(console=console) as progress:
        ...
        if source in ["google-ads", "all"]:
            campaigns.extend([
                {"source": "Google Ads", "id": "123456789", "name": "Search Campaign Q4", "status": "ENABLED", "budget": "$5,000"},
                {"source": "Google Ads", "id": "987654321", "name": "Shopping Campaign", "status": "PAUSED", "budget": "$3,000"},
                {"source": "Google Ads", "id": "456789123", "name": "Display Remarketing", "status": "ENABLED", "budget": "$2,500"},
            ])
        
        if source in ["ga4", "all"]:
            campaigns.extend([
                {"source": "GA4", "id": "ga4_001", "name": "Organic Traffic", "status": "ACTIVE", "budget": "N/A"},
                {"source": "GA4", "id": "ga4_002", "name": "Paid Search Traffic", "status": "ACTIVE", "budget": "N/A"},
                {"source": "GA4", "id": "ga4_003", "name": "Social Media Traffic", "status": "ACTIVE", "budget": "N/A"},
            ])
...
def validate_sources(
    fix_permissions: bool = typer.Option(False, "--fix", help="Attempt to fix permission issues")
):
    """
...
def schema(
    ...
    table: Optional[str] = typer.Option(None, "--table", help="Specific table/report to describe"),
...
@app.command()
def sync(
    source: str = typer.Argument("all", help="Data source to sync (ga4, google-ads, all)"),
    incremental: bool = typer.Option(True, "--incremental/--full", help="Incremental or full sync"),
    schedule: Optional[str] = typer.Option(None, "--schedule", help="Schedule sync (hourly, daily, weekly)"),
    output_dir: Optional[Path] = typer.Option(None, "--output-dir", help="Directory for synced data")
):
    """
...
def _export_ga4_data(config: Config, start_date: datetime, end_date: datetime, 
                    output_file: Path, format: str, metrics: Optional[List[str]], 
                    campaign_id: Optional[str], include_segments: bool):
...
def _export_google_ads_data(config: Config, start_date: datetime, end_date: datetime,
                           output_file: Path, format: str, metrics: Optional[List[str]],
...
def _get_data_schema(source: str, table: Optional[str]) -> dict:
