...
from .commands import data, analytics, optimization, report
from ..core.config import Config, load_config
from ..core.logging import initialize_logging, get_logger
from ..core.monitoring import system_monitor
from ..core.error_handlers import handle_errors, error_reporter
from ..core.exceptions import ConfigurationError, MarketingAIAgentError
...
def main(
    ...
    try:
        ...
        logger.info("Marketing AI Agent starting", extra={
            "version": "1.0.0",
            "config_file": str(config_file) if config_file else "default",
            "verbose": verbose,
            "output_dir": str(output_dir) if output_dir else config.output.base_directory,
...
def monitor(
    ...
    metrics_interval: int = typer.Option(60, "--metrics-interval", help="Metrics collection interval (seconds)"),
    health_interval: int = typer.Option(300, "--health-interval", help="Health check interval (seconds)"),
    export_metrics: Optional[Path] = typer.Option(None, "--export-metrics", help="Export metrics to file")
):
    """Manage system monitoring."""
    
    if start and stop:
        typer.secho("❌ Cannot start and stop monitoring at the same time", fg=typer.colors.RED)
        raise typer.Exit(1)
    ...
    if start:
        ...
        try:
            ...
            typer.secho(f"📊 Metrics exported to {export_metrics}", fg=typer.colors.GREEN)
        except Exception as e:
        ...
        if status_report["current_metrics"]:
            ...
            if avg_metrics:
                ...
                typer.echo(f"   Memory: {avg_metrics.get('avg_memory_percent', 0):.1f}%")
                typer.echo(f"   Disk: {avg_metrics.get('avg_disk_usage_percent', 0):.1f}%")
...
@handle_errors(reraise=False, report=True)
def errors(
    show_recent: int = typer.Option(10, "--recent", "-r", help="Number of recent errors to show"),
    show_summary: bool = typer.Option(False, "--summary", "-s", help="Show error summary"),
    export: Optional[Path] = typer.Option(None, "--export", help="Export error report to file")
):
    ...
    try:
        ...
        if show_summary or not error_summary["recent_errors"]:
            ...
            if error_summary["error_counts"]:
                ...
                if error_summary["most_common_error"]:
                    ...
                    typer.echo(f"   Most common: {most_common[0]} ({most_common[1]} occurrences)")
            else:
        ...
        if error_summary["recent_errors"] and not show_summary:
            ...
            for error in recent_errors:
                ...
                typer.secho(f"   • [{error['timestamp'][:19]}] {error['error_type']}: {error['message']}", 
                           fg=severity_color)
...
@app.command()
def init(
    directory: Path = typer.Argument(".", help="Directory to initialize (default: current directory)"),
...
"""
    ...
    typer.echo(f"🔐 Copy .env.example to .env and add your credentials")
...
def config(
    ...
    if show:
        if current_config:
            ...
            typer.echo(f"Default Date Range: {getattr(current_config.analytics, 'default_date_range', '30d')}")
        else:
    ...
    if validate:
        if current_config:
            ...
            if output_dir.exists():
                typer.secho(f"✅ Output directory exists: {output_dir}", fg=typer.colors.GREEN)
            else:
                typer.secho(f"⚠️  Output directory will be created: {output_dir}", fg=typer.colors.YELLOW)
        else:
...
def status(
    ...
    export: Optional[Path] = typer.Option(None, "--export", help="Export status report to file")
):
    ...
    try:
        ...
        if current_config:
            ...
            if output_dir.exists():
                ...
                typer.secho(f"⚠️  Output Directory: {output_dir} (will be created)", fg=typer.colors.YELLOW)
        ...
        if detailed:
            ...
            for check_name, check_result in status_report["health_checks"].items():
                status_icon = {"healthy": "✅", "warning": "⚠️", "critical": "❌"}.get(check_result["status"], "❓")
