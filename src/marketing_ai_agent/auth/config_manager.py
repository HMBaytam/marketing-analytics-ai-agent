...
class GoogleAdsConfig(BaseModel):
    ...
    login_customer_id: Optional[str] = Field(default=None, description="Manager account customer ID")
...
class AnthropicConfig(BaseModel):
    ...
    model: str = Field(default="claude-3-sonnet-20240229", description="Default model to use")
...
class AppConfig(BaseModel):
    ...
    rate_limit_requests_per_minute: int = Field(default=60, description="Rate limit for API requests")
...
class ConfigManager:
    ...
    def load_google_ads_config(self) -> GoogleAdsConfig:
        ...
        try:
            config = GoogleAdsConfig(
                developer_token=self._get_env_var("GOOGLE_ADS_DEVELOPER_TOKEN", required=True),
                client_id=self._get_env_var("GOOGLE_ADS_CLIENT_ID", required=True),
                client_secret=self._get_env_var("GOOGLE_ADS_CLIENT_SECRET", required=True),
    ...
    def load_google_analytics_config(self) -> GoogleAnalyticsConfig:
        ...
        try:
            config = GoogleAnalyticsConfig(
                property_id=self._get_env_var("GOOGLE_ANALYTICS_PROPERTY_ID", required=True),
    ...
    def save_config_template(self, path: Optional[Union[str, Path]] = None) -> None:
        """
