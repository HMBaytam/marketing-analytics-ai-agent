...
class OAuth2Config(BaseModel):
    ...
    redirect_uri: str = Field(default="http://localhost:8080", description="OAuth2 redirect URI")
...
class OAuth2Manager(ABC):
    ...
    def __init__(self, config: OAuth2Config, credentials_path: Optional[Union[str, Path]] = None):
        """
    ...
    def store_credentials(self, credentials: Credentials, account_id: str = "default") -> None:
        """
        ...
        try:
            ...
            logger.info(f"Stored credentials for {self.service_name} account: {account_id}")
            
        except Exception as e:
            logger.error(f"Failed to store credentials: {e}")
            raise
    
    def load_credentials(self, account_id: str = "default") -> Optional[OAuth2Credentials]:
        """
        ...
        try:
            ...
            if not stored_data:
                logger.info(f"No stored credentials found for {self.service_name} account: {account_id}")
                return None
            ...
            logger.info(f"Loaded credentials for {self.service_name} account: {account_id}")
            return credentials
    ...
    def load_service_account_credentials(self) -> Optional[ServiceAccountCredentials]:
        """
    ...
    def refresh_credentials(self, credentials: OAuth2Credentials, account_id: str = "default") -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Successfully refreshed credentials for {self.service_name} account: {account_id}")
            return True
    ...
    def get_valid_credentials(self, account_id: str = "default") -> Optional[Credentials]:
        """
        ...
        if not oauth_creds:
            logger.warning(f"No credentials found for {self.service_name} account: {account_id}")
            return None
        
        # Check if refresh is needed
        if oauth_creds.expired or (
            oauth_creds.expiry and 
            oauth_creds.expiry < datetime.utcnow() + timedelta(minutes=5)
        ):
    ...
    def complete_oauth_flow(self, authorization_code: str, account_id: str = "default") -> bool:
        """
        ...
        try:
            ...
            logger.info(f"Successfully completed OAuth flow for {self.service_name} account: {account_id}")
            return True
    ...
    def revoke_credentials(self, account_id: str = "default") -> bool:
        ...
        try:
            ...
            logger.info(f"Revoked credentials for {self.service_name} account: {account_id}")
            return True
