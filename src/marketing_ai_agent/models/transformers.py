...
from ..models.metrics import DailyMetrics, HourlyMetrics
from ..models.analytics import (
    TrafficData, ConversionEvent, ChannelPerformance,
    ChannelGrouping, DeviceCategory, EventCategory
)
...
class GoogleAdsTransformer:
    ...
    @staticmethod
    def transform_campaign(row: Any, customer_id: str) -> Optional[Campaign]:
        """
        ...
        try:
            ...
            if hasattr(campaign, 'advertising_channel_type'):
                try:
                    channel_type = AdvertisingChannelType(campaign.advertising_channel_type.name)
                except ValueError:
                    logger.warning(f"Unknown channel type: {campaign.advertising_channel_type.name}")
    ...
    @staticmethod
    def transform_ad_group(row: Any, customer_id: str) -> Optional[AdGroup]:
        """
    ...
    @staticmethod
    def transform_keyword(row: Any, customer_id: str) -> Optional[Keyword]:
        """
        ...
        try:
            ...
            if hasattr(keyword.keyword, 'match_type'):
                try:
                    ...
                    logger.warning(f"Unknown match type: {keyword.keyword.match_type.name}")
            ...
            if hasattr(keyword, 'quality_info') and keyword.quality_info:
                keyword_model.quality_score = getattr(keyword.quality_info, 'quality_score', None)
    ...
    @staticmethod
    def transform_performance_data(row: Any, customer_id: str) -> Optional[CampaignPerformance]:
        """
...
class GA4Transformer:
    ...
    @staticmethod
    def transform_traffic_data(row: Any, property_id: str) -> Optional[TrafficData]:
        """
        ...
        try:
            ...
            traffic_date = datetime.strptime(dimensions.get("dim_0", ""), '%Y%m%d').date()
            ...
            traffic_model = TrafficData(
                ...
                device_category=GA4Transformer._parse_device_category(dimensions.get("dim_2")),
                channel_grouping=GA4Transformer._parse_channel_grouping(dimensions.get("dim_3")),
                ...
                bounce_rate=float(metrics.get("metric_4", 0)) if metrics.get("metric_4") else None,
                average_session_duration=float(metrics.get("metric_5", 0)) if metrics.get("metric_5") else None
            )
    ...
    @staticmethod
    def transform_conversion_event(row: Any, property_id: str) -> Optional[ConversionEvent]:
        """
        ...
        try:
            ...
            conversion_model = ConversionEvent(
                ...
                channel_grouping=GA4Transformer._parse_channel_grouping(dimensions.get("dim_2")),
                conversions=float(metrics.get("metric_0", 0)),
                total_revenue=Decimal(metrics.get("metric_1", 0)) if metrics.get("metric_1") else None,
                purchase_revenue=Decimal(metrics.get("metric_2", 0)) if metrics.get("metric_2") else None,
    ...
    @staticmethod
    def _parse_device_category(value: Optional[str]) -> Optional[DeviceCategory]:
    ...
    @staticmethod
    def _parse_channel_grouping(value: Optional[str]) -> Optional[ChannelGrouping]:
        ...
        for campaign in campaigns:
            try:
                ...
                if not campaign.id or not campaign.name:
                    logger.warning(f"Skipping campaign with missing ID or name: {campaign}")
                    continue
