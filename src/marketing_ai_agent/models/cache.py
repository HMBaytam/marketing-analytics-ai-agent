...
class CacheConfig(BaseModel):
    ...
    max_cache_size: int = Field(
        default=100, description="Maximum number of cached items per type"
    )
    cleanup_interval: int = Field(
        default=86400, description="Cache cleanup interval in seconds (24 hours)"
    )
...
class FileCache:
    ...
    async def set(
        self, cache_type: str, key: str, data: Any, ttl: int | None = None
    ) -> bool:
        """
        Store item in cache.
    ...
    async def get_stats(self, cache_type: str | None = None) -> dict[str, Any]:
        """
        ...
        cache_types = (
            [cache_type] if cache_type else ["google_ads", "ga4", "transformed"]
        )

        for ct in cache_types:
            cache_dir = self.cache_dir / ct
            if not cache_dir.exists():
                stats["types"][ct] = {"entries": 0, "total_size": 0, "expired": 0}
                continue
...
class CacheManager:
    ...
    async def get_google_ads_campaigns(
        self, customer_id: str, query_params: dict[str, Any]
    ) -> list[Any] | None:
    ...
    async def get_ga4_traffic(
        self, property_id: str, query_params: dict[str, Any]
    ) -> Any | None:
