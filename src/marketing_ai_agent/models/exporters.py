...
from .analytics import ConversionEvent, TrafficData
from .campaign import AdGroup, Campaign, CampaignPerformance, Keyword
...
class DataExporter:
            ...
            if hasattr(item, "dict"):
                ...
                for key, value in item_dict.items():
                    if hasattr(value, "__float__"):
                        try:
                            item_dict[key] = round(
                                float(value), self.config.decimal_places
                            )
    ...
    def export_to_json(
        self, data: list[Any], filename: str, pretty: bool = True
    ) -> str:
        """
    ...
    def export_to_excel(
        self, data: list[Any], filename: str, sheet_name: str = "Data"
    ) -> str:
        """
    ...
    def export_performance_data(
        self,
        performance_data: list[DailyMetrics | CampaignPerformance],
        format_type: str = "csv",
    ...
    def export_analytics_data(
        self,
        analytics_data: list[TrafficData | ConversionEvent],
        format_type: str = "csv",
...
class MultiSheetExporter:
    ...
    def export_marketing_report(
        ...
        ad_groups: list[AdGroup] | None = None,
        keywords: list[Keyword] | None = None,
        performance_data: list[DailyMetrics] | None = None,
        analytics_data: list[TrafficData] | None = None,
        ...
        try:
            ...
            with pd.ExcelWriter(file_path, engine="openpyxl") as writer:
                ...
                if ad_groups:
                    ...
                    pd.DataFrame(ad_groups_data).to_excel(
                        writer, sheet_name="Ad Groups", index=False
                    )

                if keywords:
                    keywords_data = self.exporter._prepare_data_for_export(keywords)
                    pd.DataFrame(keywords_data).to_excel(
                        writer, sheet_name="Keywords", index=False
                    )

                if performance_data:
                    performance_data_prepared = self.exporter._prepare_data_for_export(
                        performance_data
                    )
                    pd.DataFrame(performance_data_prepared).to_excel(
                        writer, sheet_name="Performance", index=False
                    )

                if analytics_data:
                    analytics_data_prepared = self.exporter._prepare_data_for_export(
                        analytics_data
                    )
                    pd.DataFrame(analytics_data_prepared).to_excel(
                        writer, sheet_name="Analytics", index=False
                    )

                # Create summary sheet
                self._create_summary_sheet(
                    writer, campaigns, performance_data, analytics_data
                )
        ...
        try:
            ...
            if campaigns:
                ...
                active_campaigns = len(
                    [c for c in campaigns if c.status.value == "ENABLED"]
                )
                    ...
                    [
                        {
                            ...
                            "Value": date_range_start.strftime(self.config.date_format),
                        },
                        {
                            "Metric": "Date Range End",
                            "Value": date_range_end.strftime(self.config.date_format),
                        },
                        {
                            "Metric": "Performance Records",
                            "Value": len(performance_data),
                        },
                    ]
                )
...
class ReportGenerator:
    ...
    def generate_campaign_performance_report(
        ...
        if format_type.lower() == "xlsx":
            ...
            return (
                self.exporter.export_to_csv(combined_data, filename)
                if format_type.lower() == "csv"
                else self.exporter.export_to_json(combined_data, filename)
            )
    ...
    def generate_analytics_overview_report(
        ...
        if format_type.lower() == "xlsx":
            return self.multi_exporter.export_marketing_report(
                analytics_data=traffic_data, filename=filename
            )
        else:
            # Combine traffic and conversion data
            combined_data = traffic_data + conversion_events
            return self.exporter.export_analytics_data(
                combined_data, format_type, filename
            )
