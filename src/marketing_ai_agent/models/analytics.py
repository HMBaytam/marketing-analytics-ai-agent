...
class TrafficData(BaseModel):
    ...
    device_category: Optional[DeviceCategory] = Field(None, description="Device category")
    channel_grouping: Optional[ChannelGrouping] = Field(None, description="Default channel grouping")
    ...
    bounce_rate: Optional[float] = Field(None, description="Bounce rate (0-1)")
    average_session_duration: Optional[float] = Field(None, description="Average session duration in seconds")
    
    # Engagement metrics
    engaged_sessions: Optional[int] = Field(None, description="Number of engaged sessions")
    engagement_rate: Optional[float] = Field(None, description="Engagement rate (0-1)")
    events_per_session: Optional[float] = Field(None, description="Average events per session")
    ...
    @property
    def pages_per_session(self) -> Optional[float]:
...
class ConversionEvent(BaseModel):
    ...
    total_revenue: Optional[Decimal] = Field(None, description="Total revenue from conversions")
    purchase_revenue: Optional[Decimal] = Field(None, description="Purchase revenue")
    event_count: int = Field(0, description="Total event count")
    
    # Conversion details
    conversion_value: Optional[Decimal] = Field(None, description="Average conversion value")
    items_purchased: Optional[int] = Field(None, description="Number of items purchased")
    transaction_id: Optional[str] = Field(None, description="Transaction ID for purchases")
    
    @property
    def average_order_value(self) -> Optional[Decimal]:
...
class ChannelPerformance(BaseModel):
    ...
    source: Optional[str] = Field(None, description="Primary source")
    medium: Optional[str] = Field(None, description="Primary medium")
    ...
    @property
    def revenue_per_user(self) -> Optional[Decimal]:
    ...
    @property
    def revenue_per_session(self) -> Optional[Decimal]:
...
class AudienceSegment(BaseModel):
    ...
    revenue: Optional[Decimal] = Field(None, description="Revenue from segment")
    
    # Engagement metrics
    average_session_duration: Optional[float] = Field(None, description="Average session duration")
    pages_per_session: Optional[float] = Field(None, description="Pages per session")
    bounce_rate: Optional[float] = Field(None, description="Bounce rate")
    
    @property
    def sessions_per_user(self) -> Optional[float]:
...
class AttributionData(BaseModel):
    ...
    # Channel attribution
    channel_contributions: List[Dict[str, Union[str, float, Decimal]]] = Field(
        default_factory=list, 
        description="Channel contribution breakdown"
    )
    
    # Campaign attribution (for paid channels)
    campaign_contributions: List[Dict[str, Union[str, float, Decimal]]] = Field(
        default_factory=list,
        description="Campaign contribution breakdown"
    )
    
    # Path analysis
    conversion_paths: List[Dict[str, Union[str, int]]] = Field(
        default_factory=list,
        description="Common conversion paths"
    )
    
    # Summary metrics
    total_conversions: float = Field(0.0, description="Total conversions attributed")
    total_revenue: Optional[Decimal] = Field(None, description="Total revenue attributed")
    unique_paths: int = Field(0, description="Number of unique conversion paths")
    average_path_length: Optional[float] = Field(None, description="Average path length to conversion")
    
    def get_channel_attribution(self, channel: str) -> Optional[Dict]:
...
class EcommerceData(BaseModel):
    ...
    total_revenue: Decimal = Field(..., description="Total revenue including tax and shipping")
    
    # Transaction attributes
    currency: str = Field(default="USD", description="Currency code")
    affiliation: Optional[str] = Field(None, description="Store or affiliation")
    coupon: Optional[str] = Field(None, description="Coupon code used")
    
    # Items
    items: List[Dict[str, Union[str, int, Decimal]]] = Field(
        default_factory=list,
        description="Items purchased in transaction"
    )
    
    # Attribution
    channel_grouping: Optional[ChannelGrouping] = Field(None, description="Attribution channel")
    source: Optional[str] = Field(None, description="Attribution source")
    medium: Optional[str] = Field(None, description="Attribution medium")
    campaign_name: Optional[str] = Field(None, description="Attribution campaign")
    ...
    @property
    def average_item_value(self) -> Optional[Decimal]:
        """Calculate average item value."""
        if self.items:
            return self.purchase_revenue / len(self.items)
        return None
    
    def get_product_performance(self) -> Dict[str, Dict[str, Union[int, Decimal]]]:
