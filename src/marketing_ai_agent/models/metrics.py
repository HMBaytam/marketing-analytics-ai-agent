...
class DailyMetrics(MetricType):
    ...
    entity_type: str = Field(
        ..., description="Entity type (campaign, ad_group, keyword)"
    )
...
class SearchTermMetrics(MetricType):
    ...
    search_term_view_status: str | None = Field(
        None, description="Search term view status"
    )
...
class AggregatedMetrics(BaseModel):
    ...
    average_cpc_micros: int | None = Field(None, description="Average CPC in micros")
    average_cost_per_conversion_micros: int | None = Field(
        None, description="Average cost per conversion"
    )
    ...
    @property
    def total_conversion_rate(self) -> float | None:
    ...
    def calculate_averages(self, daily_metrics: list[DailyMetrics]) -> None:
        ...
        conversion_rates = [
            m.conversion_rate for m in daily_metrics if m.conversion_rate is not None
        ]
...
class PerformanceTrend(BaseModel):
    ...
    data_points: list[dict[str, Date | float]] = Field(
        ..., description="Time series data points"
    )

    # Trend analysis
    trend_direction: str | None = Field(
        None, description="Trend direction (up, down, flat)"
    )
    trend_strength: float | None = Field(None, description="Trend strength (0-1)")
    change_percent: float | None = Field(
        None, description="Percentage change from start to end"
    )
    volatility: float | None = Field(None, description="Metric volatility")

    # Statistical measures
    mean_value: float | None = Field(None, description="Mean metric value")
    median_value: float | None = Field(None, description="Median metric value")
        ...
        if total_changes > 0:
            if self.trend_direction == "up":
                ...
                self.trend_strength = 1 - (
                    abs(positive_changes - negative_changes) / total_changes
                )
