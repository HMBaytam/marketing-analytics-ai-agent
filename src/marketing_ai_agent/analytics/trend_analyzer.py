...
class TrendConfig(BaseModel):
    ...
    min_data_points: int = Field(
        7, description="Minimum data points for trend analysis"
    )
    confidence_level: float = Field(
        0.95, description="Confidence level for trend significance"
    )
    seasonal_period: int = Field(
        7, description="Seasonal period in days (7 for weekly)"
    )
    ...
    significant_change_threshold: float = Field(
        0.15, description="Threshold for significant change (15%)"
    )
    anomaly_threshold: float = Field(
        2.0, description="Z-score threshold for anomaly detection"
    )
...
class TrendAnalysis(BaseModel):
    ...
    # Seasonality
    seasonal_pattern: list[float] | None = Field(
        None, description="Detected seasonal pattern"
    )
...
class TrendAnalyzer:
    ...
    def analyze_campaign_trends(
        ...
        try:
            if len(historical_metrics) < self.config.min_data_points:
                logger.warning(
                    f"Insufficient data for trend analysis: {len(historical_metrics)} points"
                )
                return {}
    ...
    def analyze_metric_trend(
        ...
        try:
            ...
            seasonal_pattern, seasonality_strength = self._detect_seasonality(
                value_array
            )

            # Create historical points
            historical_points = []
            for i, (date, value) in enumerate(zip(dates, values, strict=False)):
    ...
    def _generate_forecast(
        self, dates: list[datetime], smoothed_values: np.ndarray, slope: float
    ) -> list[TrendPoint]:
    ...
    def detect_significant_changes(
        ...
        try:
            for metric_name, analysis in trend_analyses.items():
                ...
                if (
                    abs(analysis.recent_change)
                    > self.config.significant_change_threshold * 100
                ):
                    change_type = (
                        "increase" if analysis.recent_change > 0 else "decrease"
                    )
                    significance = (
                        "high" if abs(analysis.recent_change) > 30 else "medium"
                    )
                ...
                if abs(analysis.acceleration) > 0.1:  # Arbitrary threshold
                    accel_type = (
                        "accelerating" if analysis.acceleration > 0 else "decelerating"
                    )
            ...
            return sorted(
                significant_changes, key=lambda x: x["change_magnitude"], reverse=True
            )
    ...
    def get_trend_summary(
        ...
        try:
            ...
            for _metric_name, analysis in trend_analyses.items():
            ...
            if summary["strong_trends"] > len(trend_analyses) * 0.5:
                summary["key_insights"].append(
                    "Strong trends detected in majority of metrics"
                )

            if summary["seasonal_metrics"] > 0:
                summary["key_insights"].append(
                    f"Seasonal patterns found in {summary['seasonal_metrics']} metrics"
                )
            ...
            if summary["forecast_reliability"] > 0.7:
                ...
                summary["key_insights"].append(
                    "Low forecast confidence due to volatile trends"
                )
