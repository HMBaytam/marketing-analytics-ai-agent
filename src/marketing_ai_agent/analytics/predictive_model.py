...
class PredictionConfig(BaseModel):
    ...
    # Feature engineering
    include_seasonal: bool = Field(default=True, description="Include seasonal features")
    include_lag_features: bool = Field(default=True, description="Include lag features")
    lag_periods: List[int] = Field(default=[1, 7, 14, 30], description="Lag periods to include")
    ...
    min_training_samples: int = Field(default=30, description="Minimum samples for training")
...
class PredictiveModel:
    ...
    def predict_performance(
        ...
        try:
            ...
            if len(features) < self.config.min_training_samples:
                raise ValueError(f"Insufficient data: {len(features)} < {self.config.min_training_samples}")
    ...
    def _create_features(
        ...
        for i, record in enumerate(raw_features):
            ...
            if self.config.include_seasonal and i < len(dates):
                ...
                if i == 0:
                    self.feature_names.extend(['weekday', 'day_of_month', 'month', 'quarter'])
    ...
    def _generate_insights(
        ...
        # Confidence score
        confidence_score = max(0, min(1, metrics.r2)) * 0.7 + \
                          (1 - min(1, metrics.cv_score_std)) * 0.3
        
        # Recommendations
        recommendations = []
        if metrics.r2 < 0.5:
            recommendations.append("Model accuracy is low - consider collecting more data")
        if trend_direction == "decreasing":
            recommendations.append("Declining trend detected - investigate potential causes")
        if volatility_level == "high":
            recommendations.append("High volatility predicted - increase monitoring frequency")
        if confidence_score > 0.8:
            recommendations.append("High confidence predictions - suitable for planning")
