...
class ScoringConfig(BaseModel):
    ...
    efficiency_weight: float = Field(0.3, description="Weight for cost efficiency metrics")
    ...
    target_ctr: float = Field(2.0, description="Target CTR percentage for normalization")
    target_conversion_rate: float = Field(3.0, description="Target conversion rate for normalization")
    ...
    # Volume thresholds
    min_impressions: int = Field(1000, description="Minimum impressions for reliable scoring")
...
class PerformanceScore(BaseModel):
    ...
    weaknesses: List[str] = Field(default_factory=list, description="Performance weaknesses")
    
    # Reliability indicators
    data_reliability: str = Field(..., description="Data reliability (high, medium, low)")
...
class PerformanceScorer:
    ...
    def _validate_config(self):
        ...
        if abs(total_weight - 1.0) > 0.01:
            logger.warning(f"Scoring weights sum to {total_weight:.3f}, not 1.0. Normalizing weights.")
            # Normalize weights
    ...
    def score_campaigns(self, campaigns: List[Campaign], historical_metrics: Optional[List[DailyMetrics]] = None) -> List[PerformanceScore]:
        """
        ...
        try:
            ...
            for campaign in campaigns:
                try:
                    score = self._score_campaign(campaign, metrics_lookup.get(campaign.id, []))
                    scores.append(score)
                except Exception as e:
                    logger.error(f"Failed to score campaign {campaign.id}: {e}")
                    # Create a fallback score
                    scores.append(self._create_fallback_score(campaign.id, campaign.name, "campaign"))
    ...
    def _score_campaign(self, campaign: Campaign, historical_metrics: List[DailyMetrics]) -> PerformanceScore:
        ...
        key_metrics = {
            ...
            "cpc": float(campaign.cost) / campaign.clicks if campaign.clicks > 0 else 0.0
        }
        
        # Determine strengths and weaknesses
        strengths, weaknesses = self._analyze_strengths_weaknesses(campaign, {
    ...
    def _calculate_volume_score(self, campaign: Campaign) -> float:
        ...
        try:
            ...
            impression_score = min(100, 100 * (math.log(impressions + 1) / math.log(100000)))
    ...
    def _calculate_quality_score(self, campaign: Campaign) -> float:
        ...
        try:
            ...
            if campaign.conversion_rate:
                conversion_score = min(100, 100 * (campaign.conversion_rate / self.config.target_conversion_rate))
    ...
    def _calculate_trend_score(self, historical_metrics: List[DailyMetrics]) -> float:
        ...
        try:
            ...
            conv_values = [m.conversion_rate for m in sorted_metrics if m.conversion_rate is not None]
    ...
    def _analyze_strengths_weaknesses(self, campaign: Campaign, component_scores: Dict[str, float]) -> Tuple[List[str], List[str]]:
        ...
        if campaign.conversion_rate and campaign.conversion_rate >= self.config.target_conversion_rate:
            strengths.append(f"High conversion rate ({campaign.conversion_rate:.2f}%)")
        elif campaign.conversion_rate and campaign.conversion_rate < self.config.target_conversion_rate * 0.5:
    ...
    def _create_fallback_score(self, entity_id: str, entity_name: str, entity_type: str) -> PerformanceScore:
    ...
    def get_performance_insights(self, scores: List[PerformanceScore]) -> Dict[str, Any]:
        ...
        try:
            ...
            insights = {
                "summary": {
                    ...
                    "underperformers": len([s for s in scores if s.overall_score <= 40])
                },
                "grade_distribution": {},
                "common_strengths": {},
                "common_weaknesses": {},
            ...
            for score in scores:
                ...
                insights["grade_distribution"][grade] = insights["grade_distribution"].get(grade, 0) + 1
            ...
            for strength in all_strengths:
                insights["common_strengths"][strength] = insights["common_strengths"].get(strength, 0) + 1
            
            for weakness in all_weaknesses:
                insights["common_weaknesses"][weakness] = insights["common_weaknesses"].get(weakness, 0) + 1
