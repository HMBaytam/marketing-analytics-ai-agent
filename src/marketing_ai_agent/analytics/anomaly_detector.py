...
class AnomalyConfig(BaseModel):
    ...
    z_score_threshold: float = Field(
        2.5, description="Z-score threshold for statistical anomalies"
    )
    iqr_multiplier: float = Field(
        1.5, description="IQR multiplier for outlier detection"
    )
    confidence_level: float = Field(
        0.95, description="Confidence level for anomaly detection"
    )

    # Change detection
    sudden_change_threshold: float = Field(
        0.3, description="Threshold for sudden change detection (30%)"
    )
    gradual_change_window: int = Field(
        7, description="Window size for gradual change detection"
    )

    # Volume thresholds
    min_spend_anomaly: float = Field(
        100.0, description="Minimum spend for spend anomaly detection"
    )
    min_volume_anomaly: int = Field(
        100, description="Minimum volume for volume anomaly detection"
    )

    # Pattern detection
    pattern_window: int = Field(
        14, description="Window size for pattern anomaly detection"
    )
    seasonal_window: int = Field(7, description="Window size for seasonal comparison")

    # Severity levels
    critical_multiplier: float = Field(
        3.0, description="Multiplier for critical anomaly threshold"
    )
    high_multiplier: float = Field(
        2.0, description="Multiplier for high severity threshold"
    )
...
class AnomalyResult(BaseModel):
    ...
    anomaly_type: str = Field(
        ..., description="Type of anomaly (spike, drop, trend, pattern)"
    )
    severity: str = Field(
    ...
    affected_period: tuple[datetime, datetime] = Field(
        ..., description="Period affected by anomaly"
    )
    ...
    investigation_areas: list[str] = Field(
        default_factory=list, description="Areas to investigate"
    )
...
class AnomalyDetector:
    ...
    def detect_campaign_anomalies(
        ...
        reference_campaigns: list[Campaign] | None = None,
    ) -> list[AnomalyResult]:
        """
        ...
        try:
            ...
            if len(historical_metrics) < 7:  # Need minimum data
                logger.warning(
                    f"Insufficient data for anomaly detection: {len(historical_metrics)} points"
                )
                return anomalies
            ...
            for idx in all_outliers:
                ...
                anomaly = AnomalyResult(
                    ...
                    impact_score=self._calculate_impact_score(
                        metric_name, deviation, category
                    ),
                    estimated_impact=self._estimate_impact(category, deviation),
                    immediate_actions=self._generate_immediate_actions(
                        anomaly_type, metric_name, category
                    ),
                    investigation_areas=self._generate_investigation_areas(
    ...
    def _detect_trend_anomalies(
        ...
        try:
            ...
            moving_avg = np.pad(
                moving_avg,
                (window_size // 2, len(values) - len(moving_avg) - window_size // 2),
                mode="edge",
            )
            ...
            for idx in trend_outliers:
                ...
                anomaly_type = (
                    "trend_spike" if current_value > expected_value else "trend_drop"
                )
                severity = self._determine_severity(
                    deviations[idx] / std_deviation, abs(deviation_pct)
                )
                confidence = min(
                    1.0, deviations[idx] / (mean_deviation + std_deviation)
                )
                ...
                anomaly = AnomalyResult(
                    ...
                    z_score=deviations[idx] / std_deviation
                    if std_deviation > 0
                    else 0.0,
                    detection_date=dates[idx],
                    affected_period=(dates[idx], dates[idx]),
                    impact_score=self._calculate_impact_score(
                        metric_name, deviation_pct, category
                    ),
                    ...
                    context_data={
                        "method": "trend",
                        "moving_average": float(expected_value),
                    },
                )
        ...
        try:
            ...
            for i, date in enumerate(
                ...
                if dow in dow_patterns and len(dow_patterns[dow]) > 1:
                    expected_value = np.median(
                        dow_patterns[dow][:-1]
                    )  # Exclude current value
                    dow_std = (
                        np.std(dow_patterns[dow][:-1])
                        if len(dow_patterns[dow]) > 2
                        else 0
                    )
    ...
    def _detect_sudden_changes(
        ...
        try:
            ...
            # Find sudden changes
            threshold = (
                self.config.sudden_change_threshold * 100
            ...
            for idx in sudden_changes:
                ...
                if (
                    category == "Spend"
                    and current_value < self.config.min_spend_anomaly
                ):
                    continue
                elif (
                    category == "Volume"
                    and current_value < self.config.min_volume_anomaly
                ):
                    continue

                anomaly_type = (
                    "sudden_increase" if change_pct > 0 else "sudden_decrease"
                )
                severity = self._determine_severity(
                    abs(change_pct) / 50, abs(change_pct)
                )  # Scale by 50%
                ...
                if idx + 2 < len(values):
                    next_change = (
                        pct_changes[idx + 1] if idx + 1 < len(pct_changes) else 0
                    )
                ...
                anomaly = AnomalyResult(
                    ...
                    impact_score=self._calculate_impact_score(
                        metric_name, abs(change_pct), category
                    ),
    ...
    def _determine_severity(
        self, statistical_score: float, deviation_pct: float
    ) -> str:
        """Determine anomaly severity based on statistical measures and business impact."""
        if (
            statistical_score > self.config.critical_multiplier
            or abs(deviation_pct) > 75
        ):
            return "critical"
        elif statistical_score > self.config.high_multiplier or abs(deviation_pct) > 50:
            return "high"
        elif statistical_score > 1.5 or abs(deviation_pct) > 25:
            return "medium"
        else:
            return "low"
    ...
    def _calculate_impact_score(
        self, metric_name: str, deviation_pct: float, category: str
    ) -> float:
    ...
    def _generate_immediate_actions(
        self, anomaly_type: str, metric_name: str, category: str
    ) -> list[str]:
    ...
    def _generate_investigation_areas(
        self, anomaly_type: str, metric_name: str
    ) -> list[str]:
    ...
    def get_anomaly_summary(self, anomalies: list[AnomalyResult]) -> dict[str, Any]:
        ...
        try:
            ...
            for anomaly in anomalies:
                ...
                # Entity tracking
                entity_counts[anomaly.entity_name] = (
                    entity_counts.get(anomaly.entity_name, 0) + 1
                )
            ...
            if entity_counts:
                summary["most_affected_entity"] = max(
            ...
            for anomaly in critical_anomalies[:3]:
                summary["top_concerns"].append(
                    f"Critical {anomaly.anomaly_type} in {anomaly.metric_name}"
                )

            for anomaly in high_impact_anomalies[:2]:
                if len(summary["top_concerns"]) < 5:
                    summary["top_concerns"].append(
                        f"High impact {anomaly.anomaly_type} in {anomaly.metric_name}"
                    )
