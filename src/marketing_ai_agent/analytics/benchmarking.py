...
from ..models.campaign import AdvertisingChannelType, Campaign
...
class BenchmarkConfig(BaseModel):
    ...
    min_peer_campaigns: int = Field(
        3, description="Minimum campaigns for valid benchmarking"
    )

    # Industry benchmarks (default values - should be updated with real data)
    industry_benchmarks: dict[str, dict[str, float]] = Field(
        default_factory=lambda: {
            "search": {"ctr": 3.17, "conversion_rate": 3.75, "cpc": 2.69, "cpa": 48.96},
            "display": {
                "ctr": 0.46,
    ...
    outlier_threshold: float = Field(
        2.0, description="Z-score threshold for outlier removal"
    )
...
class BenchmarkResult(BaseModel):
    ...
    metric_benchmarks: dict[str, dict[str, float]] = Field(
        default_factory=dict, description="Benchmarks by metric"
    )

    # Peer comparison
    peer_group_size: int = Field(..., description="Size of comparison peer group")
    peer_comparison: dict[str, str] = Field(
        default_factory=dict, description="Performance vs peers by metric"
    )

    # Industry comparison
    industry_comparison: dict[str, dict[str, str | float]] = Field(
        default_factory=dict, description="Performance vs industry benchmarks"
    )

    # Competitive gaps
    improvement_opportunities: list[dict[str, Any]] = Field(
        default_factory=list, description="Areas for improvement with benchmarks"
    )

    # Strengths
    competitive_advantages: list[dict[str, Any]] = Field(
        default_factory=list, description="Areas where performance exceeds benchmarks"
    )

    # Benchmark metadata
    benchmark_period: tuple[datetime, datetime] = Field(
        ..., description="Benchmarking period"
    )
...
class BenchmarkingEngine:
    ...
    def benchmark_campaigns(
        ...
        try:
            ...
            if len(comparison_campaigns) < self.config.min_peer_campaigns:
                logger.warning(
                    f"Insufficient peer campaigns for benchmarking: {len(comparison_campaigns)}"
                )
            ...
            for campaign in target_campaigns:
                try:
                    channel_type = self._get_channel_key(
                        campaign.advertising_channel_type
                    )
                    peer_campaigns = channel_groups.get(
                        channel_type, comparison_campaigns
                    )

                    if len(peer_campaigns) < self.config.min_peer_campaigns:
                        peer_campaigns = (
                            comparison_campaigns  # Fall back to all campaigns
                        )
            ...
            for metric_name, campaign_value in campaign_metrics.items():
                if metric_name in peer_stats and campaign_value is not None:
                    ...
                    percentile = self._calculate_percentile(
                        campaign_value, peer_values, metric_name
                    )
    ...
    def _group_campaigns_by_channel(
    ...
    def _calculate_peer_statistics(
        self, peer_campaigns: list[Campaign]
    ) -> dict[str, list[float]]:
    ...
    def _calculate_percentile(
        self, value: float, peer_values: list[float], metric_name: str
    ) -> float:
    ...
    def _compare_to_industry(
        self, campaign_metrics: dict[str, float | None], channel_type: str
        ...
        for metric_name, campaign_value in campaign_metrics.items():
            if metric_name in industry_benchmarks and campaign_value is not None:
                ...
                if industry_value > 0:
                    difference_pct = (
                        (campaign_value - industry_value) / industry_value
                    ) * 100
                else:
                    difference_pct = 0.0
    ...
    def _identify_improvement_opportunities(
        ...
        industry_comparison: dict[str, dict[str, str | float]],
        channel_type: str,
    ) -> list[dict[str, Any]]:
        ...
        for metric_name, benchmark_data in metric_benchmarks.items():
            if benchmark_data["percentile_rank"] < 50:  # Below median
                benchmark_data["peer_median"] - benchmark_data["value"]

                # For "lower is better" metrics
                if metric_name in ["cpc", "cpa"]:
                    if (
                        benchmark_data["value"] > benchmark_data["peer_p75"]
                    ):  # Worse than 75th percentile
                        opportunities.append(
                            {
                                ...
                                "improvement_needed": benchmark_data["value"]
                                - benchmark_data["peer_p75"],
                                "priority": "high"
                                if benchmark_data["percentile_rank"] < 25
                                else "medium",
                            }
                        )
                else:
                    if (
                        benchmark_data["value"] < benchmark_data["peer_p25"]
                    ):  # Worse than 25th percentile
                        opportunities.append(
                            {
                                "metric": metric_name,
                                "type": "peer_gap",
                                "current_value": benchmark_data["value"],
                                "target_value": benchmark_data["peer_p75"],
                                "improvement_needed": benchmark_data["peer_p75"]
        ...
        for metric_name, industry_data in industry_comparison.items():
            if isinstance(industry_data, dict) and industry_data.get("performance") in [
                "poor",
                "average",
            ]:
                opportunities.append(
                    {
                        ...
                        "priority": "high"
                        if industry_data["performance"] == "poor"
                        else "medium",
                    }
                )
        ...
        opportunities.sort(
            key=lambda x: (x["priority"] == "high", x.get("improvement_needed", 0)),
            reverse=True,
        )
                ...
                advantages.append(
                    {
                        ...
                        "strength": "strong"
                        if benchmark_data["percentile_rank"] >= 90
                        else "moderate",
                    }
                )
                ...
                advantages.append(
                    {
                        ...
                        "outperformance_percent": abs(
                            industry_data["difference_percent"]
                        ),
                        "strength": "strong"
                        if industry_data["performance"] == "excellent"
                        else "moderate",
                    }
                )
    ...
    def _get_benchmark_period(
        self, historical_metrics: list[DailyMetrics] | None
    ...
    def get_benchmarking_insights(
        ...
        try:
            ...
            for result in benchmark_results:
                ...
                insights["performance_distribution"][rank] = (
                    insights["performance_distribution"].get(rank, 0) + 1
                )
            ...
            for result in benchmark_results:
                all_advantages.extend(
                    [adv["metric"] for adv in result.competitive_advantages]
                )
                all_opportunities.extend(
                    [opp["metric"] for opp in result.improvement_opportunities]
                )
            ...
            for opportunity in all_opportunities:
                opportunity_counts[opportunity] = (
                    opportunity_counts.get(opportunity, 0) + 1
                )
            ...
            if opportunity_counts:
                insights["common_opportunities"] = sorted(
            ...
            if top_performer_pct > 50:
                insights["key_insights"].append(
                    "Strong overall performance - majority of entities are top performers"
                )
            elif top_performer_pct < 20:
                insights["key_insights"].append(
                    "Performance improvement opportunity - few entities are top performers"
                )

            if insights["common_opportunities"]:
                top_opportunity = insights["common_opportunities"][0][0]
                insights["key_insights"].append(
                    f"Focus on {top_opportunity} - most common improvement opportunity"
                )

            if insights["average_percentile"] > 75:
                insights["key_insights"].append(
                    "Above-average competitive position overall"
                )
            elif insights["average_percentile"] < 50:
                insights["key_insights"].append(
                    "Below-average competitive position - consider strategic review"
                )
