...
class MLOptimizationConfig(BaseModel):
    ...
    optimization_objective: str = Field(default="roas", description="Optimization objective")
    min_training_samples: int = Field(default=100, description="Minimum training samples")
    test_size: float = Field(default=0.2, description="Test set proportion")
    
    # Optimization constraints
    max_bid_increase: float = Field(default=0.5, description="Maximum bid increase ratio")
    max_budget_shift: float = Field(default=0.3, description="Maximum budget shift ratio")
    ...
    include_interaction_features: bool = Field(default=True, description="Include feature interactions")
    feature_selection_threshold: float = Field(default=0.01, description="Feature importance threshold")
    
    # Clustering parameters
    n_clusters: int = Field(default=5, description="Number of clusters for segmentation")
    cluster_random_state: int = Field(default=42, description="Random state for clustering")
...
class MLRecommendation(BaseModel):
    ...
    similar_campaigns: List[str] = Field(description="Similar high-performing campaigns")
...
class MLOptimizer:
    ...
    def generate_ml_recommendations(
        ...
        try:
            ...
            if len(X) < self.config.min_training_samples:
                logger.warning(f"Insufficient data for ML optimization: {len(X)} samples")
                return []
            ...
            for opt_type in OptimizationType:
                if opt_type.value in trained_models:
                    rec = self._generate_optimization_recommendation(
                        opt_type, trained_models[opt_type.value], 
                        current_features, clusters, campaign_id
                    )
                    if rec:
            ...
            logger.info(f"Generated {len(high_confidence_recs)} high-confidence ML recommendations")
    ...
    def _prepare_training_data(
        ...
        for record in historical_data:
            ...
            perf_metrics = ['ctr', 'cpc', 'cpa', 'roas', 'conversion_rate', 'impressions', 'clicks', 'spend']
            ...
            if self.config.include_temporal_features and 'date' in record:
                date = record['date'] if isinstance(record['date'], datetime) else datetime.now()
                temporal_features = [
                    date.weekday(),
                    date.hour,
                    date.day,
            ...
            if self.config.include_interaction_features and len(feature_row) >= 2:
                ...
                if len(feature_names) == len(feature_row) - 2:
                    feature_names.extend(['ctr_cpc_interaction', 'cpa_roas_interaction'])
    ...
    def _extract_current_features(self, campaign_data: List[Dict[str, Any]]) -> np.ndarray:
        ...
        if self.config.include_temporal_features:
            now = datetime.now()
            temporal_features = [now.weekday(), now.hour, now.day, now.month, now.quarter]
            feature_row.extend(temporal_features)
    ...
    def _generate_optimization_recommendation(
        ...
        try:
            ...
            impl_steps, monitoring_metrics, rollback_conditions = self._generate_implementation_details(
            ...
            change_percentage = ((recommended_value - current_value) / current_value * 100) if current_value != 0 else 0
            
            return MLRecommendation(
                optimization_type=opt_type,
                confidence_score=confidence,
                expected_impact=(best_prediction - current_prediction) / current_prediction,
                current_value=current_value,
                recommended_value=recommended_value,
                change_percentage=change_percentage,
                feature_importance=importance_dict,
                similar_campaigns=similar_campaigns,
                risk_assessment=self._assess_optimization_risk(opt_type, change_percentage),
    ...
    def _find_optimal_variation(
        ...
        best_prediction = model.predict(self.scalers['main'].transform([current_features]))[0]
        ...
        for idx in optimization_indices:
            for multiplier in [0.8, 0.9, 1.1, 1.2, 1.5]:  # Test different values
                ...
                # Ensure constraints
                test_features = self._apply_optimization_constraints(test_features, current_features, opt_type)
    ...
    def _get_optimization_feature_indices(self, opt_type: OptimizationType) -> List[int]:
    ...
    def _apply_optimization_constraints(
        ...
        for i in range(len(test_features)):
            change_ratio = test_features[i] / original_features[i] if original_features[i] != 0 else 1
            
            if opt_type == OptimizationType.BID_OPTIMIZATION:
                # Limit bid changes
                max_change = 1 + self.config.max_bid_increase
                constrained_features[i] = original_features[i] * min(change_ratio, max_change)
            elif opt_type == OptimizationType.BUDGET_ALLOCATION:
    ...
    def _calculate_recommendation_confidence(
        ...
        if hasattr(model, 'feature_importances_'):
            ...
            relevant_importance = sum(model.feature_importances_[i] for i in relevant_indices)
            importance_boost = min(relevant_importance * 2, 0.2)
            base_confidence += importance_boost
        
        return min(base_confidence, 1.0)
    
    def _get_feature_importance(self, model: Any, opt_type: OptimizationType) -> Dict[str, float]:
    ...
    def _find_similar_campaigns(self, current_features: np.ndarray, clusters: np.ndarray) -> List[str]:
    ...
    def _assess_optimization_risk(self, opt_type: OptimizationType, change_percentage: float) -> Dict[str, float]:
