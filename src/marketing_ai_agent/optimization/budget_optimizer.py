...
class BudgetConstraints(BaseModel):
    ...
    min_budget_per_channel: float = Field(default=100, description="Minimum budget per channel")
    max_budget_per_channel: Optional[float] = Field(default=None, description="Maximum budget per channel")
    
    # Relative constraints
    min_allocation_percentage: float = Field(default=0.05, description="Minimum allocation percentage")
    max_allocation_percentage: float = Field(default=0.5, description="Maximum allocation percentage")
    
    # Channel-specific constraints
    channel_constraints: Dict[str, Dict[str, float]] = Field(
        default_factory=dict, 
        description="Channel-specific min/max constraints"
    )
    
    # Change constraints
    max_change_percentage: float = Field(default=0.3, description="Maximum change from current allocation")
    preserve_top_performers: bool = Field(default=True, description="Protect top performing channels")
    
    # Time constraints
    budget_period: str = Field(default="monthly", description="Budget period")
    allow_overspend: bool = Field(default=False, description="Allow temporary overspend")
    overspend_limit: float = Field(default=0.1, description="Maximum overspend percentage")
...
class BudgetAllocation(BaseModel):
    ...
    # Channel allocations
    channel_allocations: List[ChannelAllocation] = Field(description="Individual channel allocations")
...
class BudgetOptimizer:
    ...
    def optimize_budget_allocation(
        ...
        try:
            ...
            implementation_plan = self._generate_implementation_plan(channel_allocations)
    ...
    def _prepare_channel_data(
        ...
        for channel in channels:
            ...
            channel_performance = [
                ...
                if perf.get('channel_id') == channel_id or perf.get('channel') == channel_id
            ]
    ...
    def _calculate_channel_metrics(self, performance_data: List[Dict[str, Any]]) -> Dict[str, float]:
        ...
        total_conversions = sum(record.get('conversions', 0) for record in performance_data)
        ...
        efficiency_score = min(1.0, (roas / 4.0) * (conversion_rate / 0.02) * (50.0 / cpa))
    ...
    def _equal_weight_allocation(self, channel_data: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
        ...
        return {
            channel_id: allocation_per_channel
            for channel_id in channel_data.keys()
        }
    
    def _performance_based_allocation(self, channel_data: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
    ...
    def _roi_maximization_allocation(self, channel_data: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
        ...
        # Define constraints
        constraints = [
            {'type': 'eq', 'fun': lambda x: np.sum(x) - self.constraints.total_budget}  # Budget constraint
        ]
        ...
        for i, channel_id in enumerate(channel_ids):
            min_budget = max(
                ...
                self.constraints.total_budget * self.constraints.min_allocation_percentage
            )
            max_budget = min(
                self.constraints.max_budget_per_channel or float('inf'),
                self.constraints.total_budget * self.constraints.max_allocation_percentage
            )
            bounds.append((min_budget, max_budget))
        
        # Initial guess (equal allocation)
        initial_allocation = np.full(n_channels, self.constraints.total_budget / n_channels)
        ...
        if result.success:
            ...
            logger.warning("Optimization failed, falling back to performance-based allocation")
            return self._performance_based_allocation(channel_data)
        
        return {
            channel_ids[i]: optimized_allocation[i]
            for i in range(n_channels)
        }
    
    def _risk_adjusted_allocation(self, channel_data: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
        ...
        allocations = {}
        for i, channel_id in enumerate(channel_ids):
            weight = risk_adjusted_scores[i] / total_score if total_score > 0 else 1/n_channels
            allocations[channel_id] = self.constraints.total_budget * weight
        
        return allocations
    
    def _seasonal_adjusted_allocation(self, channel_data: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
    ...
    def _marginal_efficiency_allocation(self, channel_data: Dict[str, Dict[str, Any]]) -> Dict[str, float]:
        ...
        while remaining_budget > allocation_increment:
            ...
            marginal_efficiencies[best_channel] = base_roas * (1 - (current_allocation / 10000) ** 0.5)
        ...
        if self.constraints.max_change_percentage < 1.0:
            for channel_id, allocation in constrained_allocations.items():
                ...
                if current_budget > 0:
                    ...
                    constrained_allocations[channel_id] = np.clip(allocation, min_allowed, max_allowed)
    ...
    def _create_channel_allocations(
        ...
        for channel_id, recommended_budget in allocations.items():
            ...
            change_percentage = (change_amount / current_budget * 100) if current_budget > 0 else 0
            
            # Predict new ROAS (simplified)
            current_roas = data['performance_metrics']['roas']
            budget_ratio = recommended_budget / current_budget if current_budget > 0 else 1
            ...
            channel_allocations.append(ChannelAllocation(
                ...
                expected_outcomes=self._predict_outcomes(change_percentage, predicted_roas)
            ))
        
        return channel_allocations
    
    def _calculate_overall_metrics(self, channel_allocations: List[ChannelAllocation]) -> Dict[str, float]:
        """Calculate overall portfolio metrics."""
        
        total_current_budget = sum(ch.current_budget for ch in channel_allocations)
        total_recommended_budget = sum(ch.recommended_budget for ch in channel_allocations)
        
        # Weighted average ROAS
        current_weighted_roas = sum(
            ch.current_roas * ch.current_budget for ch in channel_allocations
        ) / total_current_budget if total_current_budget > 0 else 0
        
        expected_weighted_roas = sum(
            ch.predicted_roas * ch.recommended_budget for ch in channel_allocations
        ) / total_recommended_budget if total_recommended_budget > 0 else 0
        
        roas_improvement = ((expected_weighted_roas - current_weighted_roas) / 
                           current_weighted_roas) if current_weighted_roas > 0 else 0
        ...
        proportions = [a / total for a in allocations] if total > 0 else [0] * len(allocations)
    ...
    def _generate_implementation_plan(self, channel_allocations: List[ChannelAllocation]) -> Dict[str, Any]:
        """Generate implementation plan for budget reallocation."""
        
        # Sort by change magnitude
        sorted_channels = sorted(channel_allocations, key=lambda x: abs(x.change_percentage), reverse=True)
        ...
        if any(abs(ch.change_percentage) > 20 for ch in sorted_channels):
            ...
            large_changes = [ch for ch in sorted_channels if abs(ch.change_percentage) > 20]
            phases.append({
                'phase': 1,
        ...
        medium_changes = [ch for ch in sorted_channels if 5 <= abs(ch.change_percentage) <= 20]
        if medium_changes:
            phases.append({
                'phase': 2,
    ...
    def _generate_allocation_rationale(
        ...
        if change_percentage > 20:
            ...
            return f"Budget increase to capitalize on strong performance trends"
        elif change_percentage < -20:
            return f"Budget reduction due to poor efficiency (ROAS: {performance_metrics['roas']:.2f})"
        elif change_percentage < -5:
            return f"Minor budget reduction to optimize overall portfolio performance"
        else:
    ...
    def _predict_outcomes(self, change_percentage: float, predicted_roas: float) -> List[str]:
