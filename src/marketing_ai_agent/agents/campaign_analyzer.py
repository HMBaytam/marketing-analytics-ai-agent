...
from ..models.analytics import TrafficData, ChannelGrouping
from ..models.metrics import DailyMetrics, PerformanceTrend
...
class CampaignAnalyzerAgent:
    ...
    async def analyze_campaigns(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if state.google_ads_data:
                insights["campaign_efficiency"] = await self._analyze_campaign_efficiency(state.google_ads_data)
                insights["spend_analysis"] = await self._analyze_spend_distribution(state.google_ads_data)
                insights["performance_trends"] = await self._analyze_performance_trends(state.google_ads_data)
            
            # Channel performance analysis
            if state.ga4_data:
                insights["channel_performance"] = await self._analyze_channel_performance(state.ga4_data)
                insights["traffic_quality"] = await self._analyze_traffic_quality(state.ga4_data)
            
            # Cross-channel attribution (if both data sources available)
            if state.google_ads_data and state.ga4_data:
                insights["attribution_analysis"] = await self._analyze_attribution(state.google_ads_data, state.ga4_data)
    ...
    async def _analyze_campaign_efficiency(self, campaigns: List[Campaign]) -> Dict[str, Any]:
        ...
        try:
            ...
            conversion_rates = [c.conversion_rate for c in active_campaigns if c.conversion_rate is not None]
            ...
            if cpcs:
                ...
                for campaign in active_campaigns:
                    campaign_cpc = float(campaign.cost) / campaign.clicks if campaign.clicks > 0 else float('inf')
                    
                    if campaign_cpc <= median_cpc * 0.7 and campaign.conversions and campaign.conversions > 0:
    ...
    async def _analyze_spend_distribution(self, campaigns: List[Campaign]) -> Dict[str, Any]:
        ...
        try:
            ...
            for campaign in campaigns:
                ...
                if spend > 0:
                    ...
                    conversion_share = (campaign.conversions or 0) / total_conversions * 100 if total_conversions > 0 else 0
                    ...
                    campaign_spend.append({
                        ...
                        "efficiency_ratio": conversion_share / spend_share if spend_share > 0 else 0
                    })
            ...
            return {
                ...
                "pareto_analysis": {
                    ...
                    "concentration_ratio": len(pareto_campaigns) / len(campaign_spend) * 100
                }
            }
    ...
    async def _analyze_performance_trends(self, campaigns: List[Campaign]) -> Dict[str, Any]:
        """Analyze performance trends over time."""
        try:
            # This would typically require time-series data
            # For now, we'll analyze current performance distribution
            
            active_campaigns = [c for c in campaigns if c.status == CampaignStatus.ENABLED]
            ...
            return {
                ...
                "insights": {
                    ...
                    "improvement_candidates": len(performance_buckets["low_performance"]),
                    "performance_ratio": len(performance_buckets["high_performance"]) / len(active_campaigns) * 100
                }
            }
    ...
    async def _analyze_channel_performance(self, ga4_data: List[TrafficData]) -> Dict[str, Any]:
        ...
        try:
            ...
            for record in ga4_data:
                ...
                channel_metrics[channel]["conversions"] += getattr(record, 'conversions', 0) or 0
            ...
            for channel, metrics in channel_metrics.items():
                if metrics["sessions"] > 0:
                    channel_analysis[channel] = {
                        ...
                        "pages_per_session": metrics["pageviews"] / metrics["sessions"] if metrics["sessions"] > 0 else 0,
                        "conversion_rate": metrics["conversions"] / metrics["sessions"] * 100 if metrics["sessions"] > 0 else 0,
                        "user_engagement": metrics["users"] / metrics["sessions"] if metrics["sessions"] > 0 else 0
                    }
    ...
    async def _analyze_traffic_quality(self, ga4_data: List[TrafficData]) -> Dict[str, Any]:
        ...
        try:
            ...
            quality_assessment = "high" if quality_score >= 0.7 else "medium" if quality_score >= 0.4 else "low"
    ...
    async def _analyze_attribution(self, campaigns: List[Campaign], ga4_data: List[TrafficData]) -> Dict[str, Any]:
        """Analyze cross-channel attribution."""
        try:
            # Calculate paid search contribution
            paid_search_spend = sum(
                float(c.cost or 0) for c in campaigns 
                if c.advertising_channel_type.value in ["SEARCH", "SHOPPING"]
            )
            
            paid_search_conversions = sum(
                c.conversions or 0 for c in campaigns
                if c.advertising_channel_type.value in ["SEARCH", "SHOPPING"]
            )
            
            # GA4 channel data
            organic_sessions = sum(
                record.sessions for record in ga4_data
                if record.channel_grouping and "organic" in record.channel_grouping.lower()
            )
            ...
            return {
                ...
                "organic_metrics": {
                    ...
                    "session_share": organic_sessions / total_ga4_sessions * 100 if total_ga4_sessions > 0 else 0
                },
                "channel_mix": {
                    "paid_share": paid_sessions / total_ga4_sessions * 100 if total_ga4_sessions > 0 else 0,
                    "organic_share": organic_sessions / total_ga4_sessions * 100 if total_ga4_sessions > 0 else 0
                }
            }
    ...
    async def _generate_ai_insights(self, state: AgentState, analysis_results: Dict[str, Any]) -> List[str]:
        """Generate AI-powered insights from analysis results."""
        try:
            insights_prompt = self._create_insights_prompt(state.user_query, analysis_results)
    ...
    def _calculate_performance_score(self, campaign: Campaign) -> float:
        ...
        if campaign.conversion_rate:
            conv_score = min(campaign.conversion_rate / 10.0 * 0.4, 0.4)  # Cap at 10% conv rate
            score += conv_score
    ...
    def _create_insights_prompt(self, user_query: str, analysis_results: Dict[str, Any]) -> str:
