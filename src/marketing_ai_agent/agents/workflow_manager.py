...
from ..models.exporters import ReportGenerator, ExportConfig
...
class WorkflowManager:
    ...
    def __init__(self, llm_config: Optional[Dict[str, Any]] = None):
        ...
        self.orchestrator = OrchestratorAgent(
            llm=ChatAnthropic(**self.llm_config)
        )
    ...
    async def execute_query(self, user_query: str, export_format: str = "markdown") -> Dict[str, Any]:
        """
    ...
    async def _generate_report(self, state: AgentState, export_format: str) -> Optional[str]:
        ...
        try:
            if export_format.lower() == "markdown":
                ...
                logger.warning(f"Unknown export format: {export_format}, defaulting to markdown")
                return await self._generate_markdown_report(state)
    ...
    async def _generate_excel_report(self, state: AgentState) -> str:
        ...
        try:
            ...
            if campaigns or performance_data or analytics_data:
                report_path = self.report_generator.multi_exporter.export_marketing_report(
                    campaigns=campaigns,
    ...
    def _build_markdown_content(self, state: AgentState) -> str:
        ...
        # Header
        content_parts.append(f"# Marketing Analysis Report\n")
        ...
        if state.analysis_insights:
            ...
            if "campaign_efficiency" in state.analysis_insights:
                ...
                if not eff_data.get("error"):
                    ...
                    summary_points.append(f"- **Average CPC:** ${metrics.get('average_cpc', 0):.2f}")
                    summary_points.append(f"- **Average CTR:** {metrics.get('average_ctr', 0):.2f}%")
                    summary_points.append(f"- **Active Campaigns:** {eff_data.get('active_campaigns', 0)}")
            
            if "spend_analysis" in state.analysis_insights:
                spend_data = state.analysis_insights["spend_analysis"]
                if not spend_data.get("error"):
                    summary_points.append(f"- **Total Spend:** ${spend_data.get('total_spend', 0):,.2f}")
                    summary_points.append(f"- **Total Conversions:** {spend_data.get('total_conversions', 0):,}")
        ...
        # Footer
        content_parts.append("---\n")
        content_parts.append("*This report was generated by the Marketing Analytics AI Agent*\n")
    ...
    def _format_efficiency_insights(self, data: Dict[str, Any]) -> str:
        ...
        if metrics:
            ...
            content.append(f"- Average Conversion Rate: {metrics.get('average_conversion_rate', 0):.2f}%\n\n")
        
        top_performers = data.get("top_performers", [])
        if top_performers:
            content.append("**Top Performing Campaigns:**\n")
            for campaign in top_performers[:3]:
                content.append(f"- {campaign['name']}: CPC ${campaign['cpc']:.2f}, "
    ...
    def _format_spend_insights(self, data: Dict[str, Any]) -> str:
        ...
        content.append(f"**Total Conversions:** {data.get('total_conversions', 0):,}\n\n")
        ...
        if campaign_distribution:
            ...
            for campaign in campaign_distribution[:5]:
                content.append(f"- {campaign['name']}: ${campaign['spend']:,.0f} "
    ...
    def _format_channel_insights(self, data: Dict[str, Any]) -> str:
        ...
        if channel_performance:
            ...
            for channel, metrics in list(channel_performance.items())[:5]:
                content.append(f"- **{channel}:** {metrics['sessions']:,} sessions "
    ...
    async def health_check(self) -> Dict[str, Any]:
        ...
        try:
            ...
            test_state = AgentState(user_query="test query")
