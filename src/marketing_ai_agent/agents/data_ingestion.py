...
from ..models.cache import CacheConfig, CacheManager
from ..models.transformers import GA4Transformer, GoogleAdsTransformer
...
class DataIngestionAgent:
    ...
    def __init__(self, llm: ChatAnthropic | None = None):
        ...
        self.llm = llm or ChatAnthropic(
            model="claude-3-haiku-20240307", temperature=0.1, max_tokens=2000
        )
    ...
    async def _fetch_google_ads_data(
        self, state: AgentState, requirements: dict[str, Any]
    ) -> None:
        """
        ...
        try:
            ...
            if campaigns:
                ...
                for campaign_row in campaigns:
                    campaign = self.google_ads_transformer.transform_campaign(
                        campaign_row, customer_id
                    )
                    if campaign:
                ...
                logger.info(
                    f"Fetched {len(transformed_campaigns)} campaigns from Google Ads"
                )
            else:
                logger.warning("No Google Ads campaigns found")
                state.error_messages.append(
                    "No Google Ads data available for the specified period"
                )
    ...
    async def _fetch_ga4_data(
        ...
        try:
            ...
            if traffic_data:
                ...
                state.error_messages.append(
                    "No GA4 data available for the specified period"
                )
    ...
    def _create_ingestion_summary(self, state: AgentState) -> str:
        ...
        if state.performance_metrics:
            summary_parts.append(
                f"{len(state.performance_metrics)} performance metrics"
            )
        ...
        try:
            ...
            if len(quality_report["issues"]) > 3:
                ...
                quality_report["recommendations"].append(
                    "Consider expanding date range or checking data sources"
                )
            elif len(quality_report["issues"]) > 1:
                quality_report["overall_quality"] = "fair"
                quality_report["recommendations"].append(
                    "Some data gaps detected, results may be incomplete"
                )
