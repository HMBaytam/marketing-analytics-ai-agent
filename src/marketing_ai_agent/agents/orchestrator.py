...
from ..models.analytics import ConversionEvent, TrafficData
...
class AgentState(BaseModel):
    ...
    # Data storage
    google_ads_data: list[Campaign] | None = Field(
        default=None, description="Google Ads campaign data"
    )
    ga4_data: list[TrafficData] | None = Field(
        default=None, description="GA4 traffic data"
    )
    performance_metrics: list[DailyMetrics] | None = Field(
        default=None, description="Performance metrics"
    )
    conversion_events: list[ConversionEvent] | None = Field(
        default=None, description="Conversion data"
    )
    ...
    date_range: tuple[datetime, datetime] | None = Field(
        default=None, description="Analysis date range"
    )
...
class OrchestratorAgent:
    ...
    def __init__(self, llm: ChatAnthropic | None = None):
        ...
        self.llm = llm or ChatAnthropic(
            model="claude-3-sonnet-20240229", temperature=0.3, max_tokens=4000
        )
    ...
    async def finalize_results(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            logger.info(
                f"Workflow completed: {completed_count}/{total_steps} steps successful"
            )
    ...
    def _extract_date_range(self, query: str) -> tuple[datetime, datetime] | None:
