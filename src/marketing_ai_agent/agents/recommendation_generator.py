...
class RecommendationAgent:
    ...
    async def generate_recommendations(self, state: AgentState) -> AgentState:
        ...
        try:
            ...
            if not state.analysis_insights:
                state.error_messages.append("No analysis insights available for recommendation generation")
                return state
            ...
            if "performance_trends" in state.analysis_insights:
                performance_recs = await self._generate_performance_recommendations(state)
            ...
            state.recommendations = [self._format_recommendation(rec) for rec in final_recommendations]
    ...
    async def _generate_campaign_recommendations(self, state: AgentState) -> List[Recommendation]:
            ...
            if underperformers:
                for campaign in underperformers[:3]:  # Top 3 underperformers
                    recommendations.append(Recommendation(
                        ...
                        priority="high" if campaign['cpc'] > efficiency_data.get("metrics", {}).get("median_cpc", 0) * 2 else "medium",
    ...
    async def _generate_budget_recommendations(self, state: AgentState) -> List[Recommendation]:
        ...
        try:
            ...
            if concentration_ratio < 20:  # Less than 20% of campaigns driving 80% of spend
                recommendations.append(Recommendation(
            ...
            if campaign_distribution:
                ...
                inefficient_campaigns = [
                    c for c in campaign_distribution 
                    if c.get("efficiency_ratio", 0) < 0.5 and c.get("spend_share", 0) > 5
                ]
                
                if inefficient_campaigns:
                    total_inefficient_spend = sum(c["spend"] for c in inefficient_campaigns)
                    recommendations.append(Recommendation(
                        ...
                        timeline="3-4 weeks",
                        supporting_data={"inefficient_campaigns": inefficient_campaigns[:3]}
                    ))
    ...
    async def _generate_channel_recommendations(self, state: AgentState) -> List[Recommendation]:
        ...
        try:
            ...
            for channel, metrics in channel_performance.items():
                ...
                if conversion_rate < 1.0 and metrics.get("session_share", 0) > 10:  # Low conv rate, high traffic
                    recommendations.append(Recommendation(
                ...
                if pages_per_session < 1.5 and metrics.get("sessions", 0) > 100:  # Low engagement
                    recommendations.append(Recommendation(
            ...
            if top_channels:
                ...
                if top_metrics.get("conversion_rate", 0) > 2.0:  # High-performing channel
                    recommendations.append(Recommendation(
    ...
    async def _generate_performance_recommendations(self, state: AgentState) -> List[Recommendation]:
        ...
        try:
            ...
            if improvement_candidates > 3:
                recommendations.append(Recommendation(
                    ...
                    timeline="3-5 weeks",
                    supporting_data={"improvement_candidates": improvement_candidates}
                ))
    ...
    async def _generate_ai_recommendations(self, state: AgentState) -> List[Recommendation]:
        ...
        try:
            ...
            response = await self.llm.ainvoke([
                ...
                HumanMessage(content=f"Generate strategic recommendations based on the analysis for: {state.user_query}")
            ])
