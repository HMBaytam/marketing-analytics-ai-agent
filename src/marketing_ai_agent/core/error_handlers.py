            ...
            for attempt in range(config.max_attempts):
                try:
                    if attempt > 0:
                        ...
                        logger.info(
                            f"Retrying {func.__name__} in {delay:.2f}s (attempt {attempt + 1}/{config.max_attempts})"
                        )
                        time.sleep(delay)
        ...
        def wrapper(*args, **kwargs):
            if self.state == "OPEN":
                if self._should_attempt_reset():
                    ...
                    self.logger.info(
                        f"Circuit breaker for {func.__name__} transitioning to HALF_OPEN"
                    )
                else:
...
class ErrorReporter:
    ...
    def report_error(
        ...
        # Log the error
        self.logger.error(
            f"Error reported: {error_message}",
            extra={
                "error_type": error_type,
                "severity": severity,
                "context": context,
    ...
    def _handle_api_error(self, error: APIError):
        ...
        if error.status_code == 429:  # Rate limit
            ...
            self.logger.warning(
                f"Server error from {error.api_name} API: {error.status_code}"
            )

    def _handle_config_error(self, error: ConfigurationError):
        """Handle configuration errors."""
        self.logger.critical(
            f"Configuration error for key '{error.config_key}': {error.message}"
        )

    def _handle_data_validation_error(self, error: DataValidationError):
        """Handle data validation errors."""
        self.logger.warning(
            f"Data validation failed for field '{error.field_name}': {error.message}"
        )
        ...
        def wrapper(*args, **kwargs):
            try:
                if not validation_func(*args, **kwargs):
                    message = (
                        error_message or f"Input validation failed for {func.__name__}"
                    )
