...
class APIConfig(BaseModel):
    ...
    google_ads: dict[str, str] = Field(
        default_factory=dict, description="Google Ads API settings"
    )
    google_analytics: dict[str, str] = Field(
        default_factory=dict, description="Google Analytics API settings"
    )
    openai: dict[str, str] = Field(
        default_factory=dict, description="OpenAI API settings"
    )
    anthropic: dict[str, str] = Field(
        default_factory=dict, description="Anthropic API settings"
    )
...
class OutputConfig(BaseModel):
    ...
    base_directory: str = Field(
        default="./reports", description="Base output directory"
    )
...
class AnalyticsConfig(BaseModel):
    ...
    default_date_range: str = Field(
        default="30d", description="Default date range for analysis"
    )
    ...
    min_sample_size: int = Field(
        default=100, description="Minimum sample size for analysis"
    )
...
class OptimizationConfig(BaseModel):
    ...
    budget_risk_threshold: float = Field(
        default=0.1, description="Maximum budget risk for optimizations"
    )
...
def load_config(config_path: Path) -> Config:
    ...
    if config_data:
        ...
        if "api" in config_data:
            ...
            if (
                "google_analytics" in config_data["api"]
                and env_config.api.google_analytics
            ):
                config_data["api"]["google_analytics"].update(
                    env_config.api.google_analytics
                )
