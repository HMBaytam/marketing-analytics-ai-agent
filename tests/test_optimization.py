...
from marketing_ai_agent.core.exceptions import OptimizationError, DataValidationError
...
def sample_campaign_data():
    ...
    return pd.DataFrame({
        ...
        'bid_strategy': np.random.choice(['TARGET_CPA', 'TARGET_ROAS', 'MAXIMIZE_CLICKS'], 10),
...
class TestBudgetOptimizer:
    ...
    def test_maximize_conversions_optimization(self, sample_campaign_data, optimization_constraints):
        ...
        for budget in budgets.values():
            assert optimization_constraints['min_campaign_budget'] <= budget <= optimization_constraints['max_campaign_budget']
    
    def test_maximize_roas_optimization(self, sample_campaign_data, optimization_constraints):
        ...
        if 'total_roas' in expected_perf:
            assert expected_perf['total_roas'] >= optimization_constraints.get('target_roas', 0) * 0.9  # Allow 10% tolerance
    
    def test_budget_allocation_constraints(self, sample_campaign_data, optimization_constraints):
        ...
        for campaign_id, new_budget in optimized['optimized_budgets'].items():
            if campaign_id in current_budgets:
                ...
                assert change_ratio <= strict_constraints['budget_change_limit'] + 0.01  # Small tolerance
    
    def test_performance_forecasting(self, sample_campaign_data, optimization_constraints):
    ...
    def test_budget_optimization_with_seasonality(self, sample_campaign_data, optimization_constraints):
...
class TestROIOptimizer:
    ...
    def test_keyword_roi_optimization(self, sample_keyword_data):
        ...
        sample_keyword_data['revenue'] = sample_keyword_data['conversions'] * np.random.uniform(20, 200, len(sample_keyword_data))
...
class TestMLOptimizer:
    ...
    def test_ensemble_optimization(self, sample_campaign_data):
        ...
        assert ensemble_results['ensemble_score'] >= best_individual * 0.95  # Allow small tolerance
...
class TestRuleBasedOptimizer:
    ...
    def test_budget_rules_application(self, sample_campaign_data):
        ...
        sample_campaign_data['roas'] = (sample_campaign_data['revenue'] / sample_campaign_data['cost']) * 100
        sample_campaign_data['conversion_rate'] = sample_campaign_data['conversions'] / sample_campaign_data['clicks']
    ...
    def test_custom_rule_validation(self):
        ...
        assert optimizer.validate_rule(valid_rule) == True
        assert optimizer.validate_rule(invalid_rule) == False
    ...
    def test_rule_conflict_resolution(self, sample_campaign_data):
        ...
        for actions in campaign_actions.values():
            ...
            assert not ('increase_budget' in action_types and 'decrease_budget' in action_types)
    ...
    def test_sequential_testing(self):
        ...
        for day in range(14):
            daily_results.append({
                ...
                'test': {
                    'conversions': np.random.binomial(100, 0.035),  # Slightly better
                    'trials': 100
                }
            })
...
class TestOptimizationIntegration:
    ...
    def test_end_to_end_optimization_pipeline(self, sample_campaign_data, sample_keyword_data, optimization_constraints):
    ...
    def test_optimization_with_real_constraints(self, sample_campaign_data):
        ...
        roi_result = roi_optimizer.optimize_roi(
            campaign_data=sample_campaign_data,
            constraints=real_constraints
        )
        ...
        if 'camp_9' in optimized_budgets:
            original_budget = sample_campaign_data[sample_campaign_data['campaign_id'] == 'camp_9']['budget'].iloc[0]
        ...
        for priority_camp in real_constraints['priority_campaigns']:
            if priority_camp in optimized_budgets:
                assert optimized_budgets[priority_camp] >= real_constraints['min_campaign_budget']
