...
from marketing_ai_agent.core.exceptions import DataValidationError
...
def sample_time_series_data():
    ...
    values = (
        np.random.normal(100, 20, len(dates))
...
class TestTrendAnalyzer:
    ...
    def test_basic_trend_detection(self, sample_time_series_data):
        ...
        trend = analyzer.analyze_trend(
            sample_time_series_data["date"], sample_time_series_data["metric_value"]
        )
...
class TestAnomalyDetector:
    ...
    def test_statistical_anomaly_detection(self, sample_time_series_data):
        ...
        anomalies = detector.detect_anomalies(
            sample_time_series_data["metric_value"], method="statistical", threshold=2.0
        )
        ...
        if anomalies["anomaly_indices"]:
            assert all(
                0 <= idx < len(sample_time_series_data)
    ...
    def test_isolation_forest_detection(self, sample_time_series_data):
        ...
        anomalies = detector.detect_anomalies(
            features, method="isolation_forest", contamination=0.1
        )
    ...
    def test_anomaly_with_artificial_outliers(self):
        ...
        anomalies = detector.detect_anomalies(
            normal_data, method="statistical", threshold=3.0
        )
    ...
    def test_anomaly_explanation(self, sample_time_series_data):
        ...
        anomalies = detector.detect_anomalies(
            sample_time_series_data["metric_value"], method="statistical"
        )

        if anomalies["anomaly_indices"]:
            explanations = detector.explain_anomalies(
                sample_time_series_data, anomalies["anomaly_indices"]
            )
...
class TestPredictiveModel:
    ...
    def test_time_series_prediction(self, sample_time_series_data):
        ...
        future_dates = pd.date_range(
            start=sample_time_series_data["date"].max() + timedelta(days=1), periods=30
        )
...
class TestBenchmarkAnalyzer:
    ...
    def test_industry_benchmark_comparison(self, sample_performance_data):
        ...
        comparison = analyzer.compare_to_benchmark(
            sample_performance_data, industry_benchmarks, benchmark_type="industry"
        )
    ...
    def test_competitor_benchmark_comparison(self, sample_performance_data):
        ...
        comparison = analyzer.compare_to_competitors(
            sample_performance_data, competitor_data
        )
    ...
    def test_historical_benchmark_comparison(self, sample_performance_data):
        ...
        comparison = analyzer.compare_to_historical(current_data, historical_data)
    ...
    def test_benchmark_scoring(self, sample_performance_data):
        ...
        score = analyzer.calculate_benchmark_score(sample_performance_data, benchmarks)
...
class TestAnalyticsIntegration:
    ...
    def test_end_to_end_analysis_pipeline(self, sample_performance_data):
        ...
        analyzer = TrendAnalyzer()
        trend = analyzer.analyze_trend(
            sample_performance_data["date"], sample_performance_data["conversions"]
        )

        # Anomaly detection
        detector = AnomalyDetector()
        anomalies = detector.detect_anomalies(
            sample_performance_data["conversions"], method="statistical"
        )
        ...
        benchmark_comparison = benchmark_analyzer.compare_to_benchmark(
            sample_performance_data, benchmarks
        )
